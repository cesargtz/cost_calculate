...
class ReportCalculateCost(models.AbstractModel):
    ...
    def bills(self, single_date, typeinv):
            return self.env['account.invoice'].search([('type', 'in', typeinv), ('date_invoice', '=', single_date), ('state', 'in', ['open', 'paid'])], order="date_invoice")
    ...
    def get_totals(self, facturas):
            ...
            for order in sorted(keys, key=lambda x: datetime.datetime.strptime(x, '%Y-%m-%d')):
                    # _logger.error(order) #Aqui esta la fecha ordenada
                    sum_tot = 0
                    tons = 0
                    const = 0
            ...
            for order in sorted(keys, key=lambda x: datetime.datetime.strptime(x, '%Y-%m-%d')):
                    ...
                    # _logger.critical("tonteladas acomuladas: %s  -  importe acomulado: %s " % (sum_tons_total, sum_import_total))
                    if sum_tons_total > 0:
                            cost = float(sum_import_total / sum_tons_total)
                            # _logger.warning("El costo es de: %s " % cost)
